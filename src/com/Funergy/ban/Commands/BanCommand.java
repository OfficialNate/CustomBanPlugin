...
public class BanCommand extends Command {
	...
	public void execute(CommandSender sender, String[] args) {
		...
		if(args.length < 2){
			sender.sendMessage(new TextComponent(ChatColor.RED+"Usage: /ban <Playername> <Reason>"));
			return;
		}
		...
		if(p != null){
		mysql.permBan(p.getUniqueId().toString(), p.getName(), reason);
		BanHandler.addPlayer(true, p.getUniqueId().toString(), null, reason);
		mysql.disconect();
		p.disconnect(new TextComponent(ChatColor.GRAY+"You have been"+ChatColor.RED+" permbanned"+ChatColor.GRAY+" from the SP-Network."
			...
			mysql.permBan(mysql.getUUIDFromName(args[0]), args[0], reason);
			BanHandler.addPlayer(true, mysql.getUUIDFromName(args[0]), null, reason);
		...
		for(ProxiedPlayer players : ProxyServer.getInstance().getPlayers()){
			...
			if(s2.equalsIgnoreCase("Builder")||s2.equalsIgnoreCase("Mod")||s2.equalsIgnoreCase("Admin")||s2.equalsIgnoreCase("Developer")||s2.equalsIgnoreCase("Owner")){	
			 players.sendMessage(new TextComponent(ChatColor.RED+"Soulpoint: "+ChatColor.GREEN+sender.getName()+" Has banned "+ChatColor.YELLOW+args[0]));
